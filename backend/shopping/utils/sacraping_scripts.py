...
def get_products_by_graphql(category):  
    ...
    while True:
        ...
        for item in products["edges"]:
            if category == "bebidas":
                category = item.get("node")["breadcrumbList"]["itemListElement"][0]["name"]
            elif category == "frutas-y-verduras":
                category = item.get("node")["breadcrumbList"]["itemListElement"][1]["name"]
            product = {
                "id": item.get("node")["id"],
